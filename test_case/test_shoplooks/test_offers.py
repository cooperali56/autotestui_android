...
offers_data_test = ExcelControl("data/shoplooks/shoplooks_offers.xlsx").get_excel_data("offers")
# 获取yaml配置
data_yaml = YamlControl("common/config.yaml").get_yaml_data()


@allure.epic(f"{data_yaml['shoplooks_web']['project_name']}")
@allure.feature(f"模块名称: {offers_data_test[0]['项目模块']}")
class TestOffers:

    @allure.story(f"接口名称: {offers_data_test[0]['请求path']}")
    @allure.title(f"用例标题: {offers_data_test[0]['用例title']}")
    @pytest.mark.parametrize('in_data', offers_data_test)
